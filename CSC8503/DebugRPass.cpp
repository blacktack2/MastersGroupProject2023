...
void DebugRPass::Render() {
	renderer.GetConfig().enableDepthTest.Push(GL_FALSE);
	renderer.GetConfig().enableCullFace.Push(GL_FALSE);
	RenderLines();
	RenderText();
	renderer.GetConfig().enableCullFace.Pop();
	renderer.GetConfig().enableDepthTest.Pop();
}
