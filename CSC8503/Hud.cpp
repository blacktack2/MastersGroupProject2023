...
void Hud::Draw() {
	int c = 0;
	for (auto& hud : huds) {
		std::cout << hud.first << " " << hud.second->IsEnabled << std::endl;
		if (hud.second->IsEnabled) {
			std::cout << c << std::endl;
			hud.second->Draw();
			c++;
		}
