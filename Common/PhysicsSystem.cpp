...
bool NCL::CSC8503::PhysicsSystem::Contains(std::set<CollisionDetection::CollisionInfo> setToCheck, CollisionDetection::CollisionInfo val)
{
	if (!setToCheck.empty()) {
		for (auto& i : setToCheck) {
...
#else
				info.isEntered = !Contains(allCollisions, info);
#endif
				allCollisions.insert(info);
			}
		}
...
#else
				info.isEntered = !Contains(allTriggers, info);
#endif
