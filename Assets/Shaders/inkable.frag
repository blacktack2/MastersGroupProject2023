...
uniform vec3		trace[100];		// TODO: [performance issue] change this to SSBO
uniform vec3		testingPoint;

uniform int			numOfNodesInTrace;
uniform int			usingInkSea;
uniform vec3		inkSeaPosition;
...
void main(void)
{
	...
	if(hasTexture) {
		float delayFactor = 2.0f;
		float scaleFactor = 1.0f;
		float time = -(fract(globalTimer/delayFactor));
		float radius = (1.0/(200*2)) - time * scaleFactor * (1.0/(200*2));
		...
		if (true)		// explode on top plane
		{
			...
			if (usingInkSea == 1 && painted == false)
			{
				delayFactor = 2.0f;
				scaleFactor = 80.0f;
				time = -(fract(globalTimer/delayFactor));
				radius = (1.0/(200*2)) - time * scaleFactor * (1.0/(200*2));
				amplificationFactor = 6.0f;
				amplitude = 3.0/(200*2);
		
				vec4 colors = vec4(0.0,1.0,0.0,1.0);

				vec2 explosionPoint = GetTexPosOnTopFace(inkSeaPosition);
