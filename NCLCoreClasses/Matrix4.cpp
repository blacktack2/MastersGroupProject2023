...
void Matrix4::SetColumn(unsigned int column, const Vector4& val) {
	array[column][0] = val.x;
	array[column][1] = val.y;
	array[column][2] = val.z;
	array[column][3] = val.w;
}

Vector4 Matrix4::GetDiagonal() const{
	return Vector4(array[0][0], array[1][1], array[2][2], array[3][3]);
}

void Matrix4::SetDiagonal(const Vector4& in) {
	array[0][0] = in.x;
	array[1][1] = in.y;
	array[2][2] = in.z;
	array[3][3] = in.w;
...
/**
 * @brief Yoinked from the Open Source Doom 3 release - all credit goes to id software!
 */
void Matrix4::Invert() {
	float det, invDet;
...
Matrix4 Matrix4::Perspective(float znear, float zfar, float aspect, float fov) {
	...
	const float h = 1.0f / std::tan(fov*Maths::PI_OVER_360);
...
Vector3 Matrix4::operator*(const Vector3& v) const {
	...
	vec /= v.x * array[0][3] + v.y * array[1][3] + v.z * array[2][3] + array[3][3];
